...
# Default arguments used in API call
DFT_ARGS = args = {'custom_tick': '', 'start_dt': None, 'stacked': False, 'suffix': '', 'vertical': False,
                   'format': '{:<5.2f}', 'color': None, 'title': None, 'delim': '', 'filename': 'ex1.dat',
                   'width': 50, 'version': False, 'no_labels': False, 'calendar': False, 'different_scale': False, 'verbose': False}
...
def print_row(value, num_blocks, val_min, color):
    ...
    """
    if color:
        sys.stdout.write('\033[{color}m')  # Start to write colorized.
...
def draw_chart(datas, colors=None, args=DFT_ARGS, title=None, is_pert=True, do_sort=-1):
    r""" Draw the bar chart with given data/label

    :param datas: Data to draw the bar chart. Key as label and value as corresponding value.
    :type datas: dict
    :param colors: Color list to represent each bar
    :type colors: list
    :param args: Argument given for customized result
    :type args: dict
    :param title: Title to show in the drawn chart.
    :type title: str or callable object
    :param is_pert: True to calculate the percentage of each bar instead of actual value.
    :type is_pert: bool
    :param do_sort: 1 to sort desc; 2 to sort asc; others mean no sorting (default is -1)
    :type do_sort: int
    """
    data_sum = 0
    data = []
    labels = []
    for lab, dat in datas.items():
        data.append([dat])
        labels.append(lab)
        data_sum += dat

    if is_pert:
        data = list(map(lambda e: [e[0] * 100.0 / data_sum], data))
        args['format'] = '{:<5.2f}%'

    if isinstance(title, str):
        print('# ' + title.format(data_sum=data_sum) + '\n')
    elif callable(title):
        title()

    if do_sort in [1, 2]:
        reverse = True if do_sort == 1 else False
        sorted_tups = sorted(zip(list(map(lambda e: e[0], data), labels)), key=lambda t: t[0], reverse=reverse)
        data = []
        labels = []
        for dat, lab in sorted_tups:
            data.append([dat])
            labels.append(lab)
...
def chart(colors, data, args, labels):
    r"""Handle the normalization of data and the printing of the graph.

    :param colors: Color choice for each bar
    :param data: Data or each bar
    :type data: list
...
def print_categories(categories, colors):
    ...
    for i in range(len(categories)):
        if colors:
            sys.stdout.write('\033[{colors[i]}m')  # Start to write colorized.
...
def calendar_heatmap(data, labels, args):
    ...
    for day in range(7):
        ...
        for week in range(53):
            ...
            if colornum:
                sys.stdout.write('\033[{colornum}m')
